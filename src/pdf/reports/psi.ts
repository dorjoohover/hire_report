...
export class PSI {
  ...
  private getResultCached(value: string) {
    if (!this.resultCache.has(value)) {
      this.resultCache.set(value, this.result(value));
    }
    return this.resultCache.get(value);
  }

  private async getCategoriesCached(assessmentId: number) {
    if (!this.categoryCache.has(assessmentId)) {
      const categories =
        await this.answerCategoryDao.findByAssessmentId(assessmentId);
      this.categoryCache.set(assessmentId, categories);
    }
    return this.categoryCache.get(assessmentId);
  }
  ...
  async template(
    ...
    try {
      ...
      const allPsiCategories = await this.getCategoriesCached(
        exam.assessment.id,
      );
      const completeDetails = [...details];

      const existingValues = new Set(details.map((d) => d.value));
      for (const category of allPsiCategories) {
        if (!existingValues.has(category.name)) {
          completeDetails.push({
            cause: '0',
            value: category.name,
          } as ResultDetailEntity);
        }
      }
      ...
      const indicator = new Array(completeDetails.length);
      const data = new Array(completeDetails.length);
      const results = new Array(completeDetails.length);

      for (let i = 0; i < completeDetails.length; i++) {
        const detail = completeDetails[i];
        const resultData = this.getResultCached(detail.value);
      ...
      const sortedResults = [...results].sort((a, b) => b.point - a.point);

      const uniquePoints = [...new Set(sortedResults.map((res) => res.point))];
      const topScore = uniquePoints[0];
      const secondScore = uniquePoints[1];
      const thirdScore = uniquePoints[2];
      const fourthScore = uniquePoints[3];

      let topPoints = [];
      if (secondScore === thirdScore && thirdScore === fourthScore) {
        topPoints = uniquePoints.slice(0, 4);
      } else if (secondScore === thirdScore) {
        topPoints = uniquePoints.slice(0, 3);
      } else {
        topPoints = uniquePoints.slice(0, 2);
      }

      const topAgents = results.filter((res) => topPoints.includes(res.point));

      results.map((res, i) => {
        y = doc.y;
        const isTop = topPoints.includes(res.point);
        const color = isTop ? colors.orange : colors.black;

        doc
          .font(isTop ? fontBold : fontNormal)
          .fillColor(color)
          .text(`${i + 1}.`, x, y);
