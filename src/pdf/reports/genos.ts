  ...
  template = async (
    ...
    try {
      ...
      doc
        ...
        .fillColor(colors.black)
        .text(
          'Таны сэтгэл хөдлөлөө удирдах чадварыг жишиг нормын дагуу 7 чадвар тус бүр дээр 1-ээс 99-ийн хооронд харьцуулсан эрэмбэ буюу перцентиль оноогоор үнэлдэг. Энэ нь таны цэвэр оноо бус харин нийтлэг хүмүүсийн оноотой харьцуулан гаргасан оноо юм.\n\nХарьцуулсан эрэмбэ буюу перцентиль - өгөгдлийн сан дахь нийтлэг хүмүүсийн авсан оноонуудтай харьцуулахад таны авсан оноо хаана эрэмбэлэгдэж буйг зааж,  1-ээс 99-ийн хооронд байр эзлүүлдэг. Жишээлбэл таны авсан оноо бусад хүмүүстэй харьцуулахад 85-д эрэмбэлэгдэж буй бол та нийтлэг хүмүүсийн 85%-иас нь илүү өндөр оноо авсан гэж тайлбарлагдана.\n\nҮүнээс гадна таны тестийн оноог ойлгомжтой тайлбарлахын тулд нийтлэг хүмүүсийн оноотой харьцуулж “Доогуур”, “Харьцангуй доогуур”, “Дундаж”, “Харьцангуй дээгүүр”, “Дээгүүр” гэсэн 5 бүлэгт хуваасан. Хялбаршуулсан онооны системийн тайлбарыг дараах хүснэгтээс дэлгэрэнгүй харна уу.',
          { align: 'justify' },
        )
        .moveDown(1);
      ...
      const firstColWidth = tableWidth * 0.26;
      const shortColWidth = tableWidth * 0.12;
      const remainingColWidth = (tableWidth * 0.5) / 3;

      let startX = marginX;
      let startY = doc.y;

      for (let row = 0; row < tableData.length; row++) {
        const currentRowHeight = 36;

        for (let col = 0; col < tableData[row].length; col++) {
          let colWidth;
          if (col === 0) colWidth = firstColWidth;
          else if (col === 1 || col === 5) colWidth = shortColWidth;
          else colWidth = remainingColWidth;

          let x = startX;
          for (let i = 0; i < col; i++) {
            if (i === 0) x += firstColWidth;
            else if (i === 1 || i === 5) x += shortColWidth;
            else x += remainingColWidth;
          }
      ...
      doc
        ...
        .font(fontBold)
        .fontSize(13)
        .text('Давуу тал болон хөгжүүлэх боломжууд', marginX, doc.y + 20)
        .moveDown(0.5);
      ...
      for (let index = 0; index < results.length; index++) {
        ...
        let category = '';
        if (res.point >= 0 && res.point <= res.p1) {
          category = 'Доогуур';
        } else if (res.point >= res.p1 + 0.1 && res.point <= res.p2) {
          category = 'Харьцангуй доогуур';
        } else if (res.point >= res.p2 + 0.1 && res.point <= res.p3) {
          category = 'Хэвийн хэмжээнд';
        } else if (res.point >= res.p3 + 0.1 && res.point <= res.p4) {
          category = 'Харьцангуй дээгүүр';
        } else if (res.point >= res.p4 + 0.1 && res.point <= 50) {
          category = 'Дээгүүр';
        }
